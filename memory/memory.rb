...
class MemoryGame
  ...
  def compare_guess(new_guess)
    if previous_guess
      if previous_guess == new_guess
        begin
          raise StandardError.new("Same position guessed.")
      ...
      elsif match?(previous_guess, new_guess)
  ...
  def get_player_input
    ...
    begin
      ...
      raise StandardError.new("Position outside bounds.") unless valid_pos?(pos)
    rescue StandardError => e
      puts e.message
    retry
    ...
    #   if pos && !valid_pos?(pos)
    #     raise StandardError.new("Position outside bounds.")
    #   end
    # rescue StandardError => e
    #   puts e.message
    #   pos = player.get_input
